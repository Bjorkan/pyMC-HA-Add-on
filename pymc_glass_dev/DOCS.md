# Home Assistant Add-on: pyMC Glass Dev

## About

This add-on builds and wraps the upstream `pyMC-Glass` `dev` branch as a
...
This is the development-tracking add-on. It is intentionally built from the
upstream `dev` branch, not a stable release tag.
...
2. Install the `pyMC Glass Dev` add-on.
...
4. Edit the add-on config file `config.yaml` in the add-on's own config folder.
   You are looking for a folder matching `addon_config/*_pymc_glass_dev`.
...
- Upstream branch used by this add-on: `dev`
